...
def run_s4_stream(datasets):
    MAX_INITIAL = 500_000  # INITIAL TRAINING SAMPLE SIZE
    MAX_BATCHES = 500 # MAX NUMBER OF BATCHES
    RETRAIN_ON_EACH_BATCH = True
    RETRAIN_INTERVAL = 5  # EVERY N-th BATCH RETRAINING
    RETRAIN_ON_DROP_THRESH = 0.05  # DROP LIMIT
    BATCH_SIZE = 1000
    WINDOW_SIZE = 100_000  # SLIDING WINDOW SIZE
    ...
    for test_name in dataset_names:
        ...
        if len(combined_train) > MAX_INITIAL: #IF TWO TRAINING DASETS TOGETHER ARE BIGGER THAN 500 000 SAMPLES WE GET THAT NUMBER FROM BOTH OF THEM RANDOMLY SELECTED
        ...
        n_batches = int(np.ceil(len(X_stream) / BATCH_SIZE))
        n_batches = min(n_batches, MAX_BATCHES)
        for i in range(n_batches):
            start_idx = i * BATCH_SIZE
            end_idx = min((i + 1) * BATCH_SIZE, len(X_stream))
            ...
            for model_name, model in models.items():
                ...
                if do_retrain:
                    ...
                    current_X = pd.concat([old_X, X_batch], ignore_index=True) #SLIDING FINDOW FIRST IN FIRST OUT (FIFO)
    ...
    if stream_results:
        ...
        for stream in unique_streams:
            ...
            for model in subset_stream['Model'].unique():
                ...
                print( f"{model:<25} | {mean_val:.4f}   | {max_val:.4f}   ({batch_max:>3}) | {min_val:.4f}   ({batch_min:>3})")
        print("=" * 80 + "\n")
